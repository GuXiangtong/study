...
def list_questions():
    ...
    exams = get_all_exams()
    questions = get_questions_filtered(subject_id=subject_id, exam_id=exam_id, search=search)
...
def update_sub(sub_question_id):
    ...
    sq = get_sub_question(sub_question_id)
...
def delete_sub(sub_question_id):
    sq = get_sub_question(sub_question_id)
    qid = sq['question_id'] if sq else None
    delete_sub_question(sub_question_id)
    flash('子问题已删除', 'success')
    if qid:
        return redirect(url_for('questions.detail', question_id=qid))
    return redirect(url_for('questions.list_questions'))
