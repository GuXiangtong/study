...
2. 只整理格式，严禁根据题意推断、联想或凭记忆补全任何原文中不存在的文字、条件、数据或答案；不要回答题目或添加解析
3. 数学公式使用 LaTeX 语法（$...$ 或 $$...$$）
4. 如果题目包含子问题如(1)(2)(3)，全部保留在 content 中
5. 如果原始分题有误（如将一道题拆成多道），合并为一道
6. 如果一道题被错误合并，拆分为多道
...
## 严格转录（最重要，优先级高于其他所有规则）
- 只转录图片中真实、清晰可见的文字，逐字照抄，不要改写、润色或"补充完整"。
- 严禁根据题意推断、联想，或凭记忆/常识补全图片上不存在的任何文字、条件、数据、公式、选项或答案。
- 不要回答题目，不要添加解析、提示、翻译或任何说明性文字。
- 图片中看不清、被遮挡或残缺的部分，用 [模糊] 或 [缺失] 标注，绝不猜测填充。
- 只识别当前图片内的内容；图片没有的就不要出现在结果里。
...
7. 保留所有原始文字，不得删减、改写，也不得增加或补全任何 OCR 原文中不存在的内容（严禁根据题意推断或凭记忆补全）
...
## 注意事项
- 识别所有题目：选择题、填空题、解答题
- 只转录图片中真实、清晰可见的文字，逐字照抄；严禁推断、联想或凭记忆补全图片上不存在的文字、条件、数据、选项或答案
- 不要回答题目、不要添加解析或说明；看不清或残缺处用 [模糊] 标注，绝不猜测填充
...
def _kimi_vision_post(system_prompt, image_path, user_message):
    ...
    if not MOONSHOT_API_KEY:
        raise RuntimeError('Kimi 视觉识别需要配置 MOONSHOT_API_KEY，请在 .claude/apikey 中填写。')
